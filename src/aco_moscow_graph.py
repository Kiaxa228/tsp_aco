...
def ant_colony_optimization(g, verbose=True, iterations = 100, ants_per_iteration = 50, q = None, degradation_factor = .9, use_inertia = False, run_experiment_break=False, run_experiment_artificial_good_cycle=False):
    ...
    scooters_power = np.random.randint(10, 50, 150)
    ...
    for iteration in range(iterations):
        ...
        cycles = [traverse_graph(g, 11, scooters_power) for _ in range(ants_per_iteration)]
...
def traverse_graph(g, source_node, scooters_p):
    ...
    steps = 0
    current = source_node
    remaining_nodes = set([i for i in range(10, g.nodes) if i != source_node])
    remaining_stations = set([i for i in range(10)])
    total_length = 0
    area_power = 0.4
    cur_accums = 20
    ...
    while area_power < 80:
        ...
        if cur_accums > 0:
            denominator = calculate_denominator(g, current, remaining_nodes)

            for node in range(10, g.nodes):
                if visited[node] != 0:
                    sediment = max(g.intensity[current][node], 1e-5)
                    v = (sediment**0.9 * (1 / g.distance[current][node]) ** beta) / denominator
                    jumps_neighbors.append(node)
                    jumps_values.append(v)
        else:
            denominator = calculate_denominator(g, current, remaining_stations)
